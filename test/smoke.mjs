...
(async () => {
  ...
  // below leave cue points and a played-through crate behind, and on a second run against the same server
  ...
  for (const item of dj.state.crate) dj.cmd({ action: 'crate.remove', id: item.id });
  for (const item of dj.state.requests) dj.cmd({ action: 'request.reject', id: item.id });
  ...
  await dj.waitFor((s) => s.crate.length === 0 && s.requests.length === 0 && !s.decks[0].video && !s.decks[1].video);
  ...
  dj.cmd({ action: 'crate.add', video: { ...vid, id: '' } });
  await dj.waitFor((s) => s.crate.length === 1);
  const qid = dj.state.crate[0].id;
  ok('crate.add assigns an id', !!qid, qid);
  ok('a fresh crate item is unplayed', dj.state.crate[0].playedAt === 0);
  dj.cmd({ action: 'crate.load', id: qid, deck: 'b' });
  // Deck B already holds this video from the mixer checks, so wait on the crate ENTRY id landing
  // there - `videoId` would already be true and the assertion would race the command.
  await dj.waitFor((s) => s.decks[1].video?.id === qid);
  ok('crate.load keeps the item in the crate and stamps it played',
    dj.state.crate.length === 1 && dj.state.crate[0].playedAt > 0,
    JSON.stringify({ len: dj.state.crate.length, playedAt: dj.state.crate[0].playedAt }));
  dj.cmd({ action: 'crate.reset', id: qid });
  await dj.waitFor((s) => s.crate[0].playedAt === 0);
  ok('crate.reset puts an item back in auto-advance\'s path', true);
  ...
  const req = dj.state.requests[0];
  ok('a listener request lands in requests, not the crate', dj.state.crate.length === 0 && req.videoId === VID);
  ok('a request is stripped of any plan', req.plan.kind === '' && req.plan.cueIn === 0);

  crowd.send({ t: 'request', video: { videoId: VID, title: 'again' } });
  await sleep(250);
  ok('a second request from the same listener is refused', dj.state.requests.length === 1,
    'requests=' + dj.state.requests.length);

  dj.cmd({ action: 'request.approve', id: req.id });
  await dj.waitFor((s) => s.crate.length === 1);
  ok('request.approve moves it into the crate',
    dj.state.requests.length === 0 && dj.state.crate[0].videoId === VID && dj.state.crate[0].playedAt === 0);
  dj.cmd({ action: 'crate.remove', id: dj.state.crate[0].id });
  await dj.waitFor((s) => s.crate.length === 0);

  // ---------------------------------------------------------------- auto-advance (infinite set)
  // Everything below runs in seconds by giving every crate item a 3s out point and a 1s planned
  ...
  const titles = (s) => s.crate.map((v) => v.title);
  /* What auto-advance can still reach. The crate itself never shrinks as the set runs. */
  const unplayed = (s) => s.crate.filter((v) => v.playedAt === 0);
  ...
  await dj.waitFor((s) => !s.decks[0].video && !s.decks[1].video && s.crate.length === 0);
  ok('decks and crate are clear before the auto-advance run', true);

  dj.cmd({ action: 'crate.addMany', videos: [track('A1'), track('A2'), track('A3')] });
  await dj.waitFor((s) => s.crate.length === 3);
  ok('crate.addMany preserves order', titles(dj.state).join(',') === 'A1,A2,A3', titles(dj.state).join(','));
  ok('crate.addMany assigns distinct ids', new Set(dj.state.crate.map((v) => v.id)).size === 3);
  ok('crate.addMany keeps each item plan', dj.state.crate.every((v) => v.plan.cueOut === 3 && v.plan.durationMs === 1000));

  dj.cmd({ action: 'crate.addMany', videos: [{ ...vid, id: '', videoId: 'nope' }, track('A4')] });
  await dj.waitFor((s) => s.crate.length === 4);
  ok('crate.addMany skips unusable rows and keeps the rest', titles(dj.state).join(',') === 'A1,A2,A3,A4');
  dj.cmd({ action: 'crate.remove', id: dj.state.crate[3].id });
  await dj.waitFor((s) => s.crate.length === 3);

  // --- crate.plan is a PARTIAL update
  const planned = dj.state.crate[1].id;
  dj.cmd({ action: 'crate.plan', id: planned, plan: { kind: 'bassSwap', durationMs: 2000, cueIn: 5, cueOut: 30 } });
  await dj.waitFor((s) => s.crate[1].plan.kind === 'bassSwap');
  ok('crate.plan sets a full plan',
    JSON.stringify(dj.state.crate[1].plan) === JSON.stringify({ kind: 'bassSwap', durationMs: 2000, cueIn: 5, cueOut: 30 }),
    JSON.stringify(dj.state.crate[1].plan));

  dj.cmd({ action: 'crate.plan', id: planned, plan: { durationMs: 1500 } });
  await dj.waitFor((s) => s.crate[1].plan.durationMs === 1500);
  const p2 = dj.state.crate[1].plan;
  ok('a partial crate.plan does not clobber sibling fields',
    p2.kind === 'bassSwap' && p2.cueIn === 5 && p2.cueOut === 30, JSON.stringify(p2));

  dj.cmd({ action: 'crate.plan', id: planned, plan: { cueIn: 9 } });
  await dj.waitFor((s) => s.crate[1].plan.cueIn === 9);
  const p3 = dj.state.crate[1].plan;
  ok('a partial crate.plan keeps the other cue point',
    p3.cueOut === 30 && p3.durationMs === 1500 && p3.kind === 'bassSwap', JSON.stringify(p3));

  dj.cmd({ action: 'crate.plan', id: planned, plan: { durationMs: 10 } });
  dj.cmd({ action: 'crate.plan', id: planned, plan: { kind: 'nonsense' } });
  dj.cmd({ action: 'crate.plan', id: planned, plan: { cueOut: 2 } });
  await sleep(250);
  const p4 = dj.state.crate[1].plan;
  ok('an invalid crate.plan patch changes nothing',
    p4.durationMs === 1500 && p4.kind === 'bassSwap' && p4.cueOut === 30, JSON.stringify(p4));

  // Put item 2 back on the fast schedule for the run.
  dj.cmd({ action: 'crate.plan', id: planned, plan: { kind: 'crossfade', durationMs: 1000, cueIn: 0, cueOut: 3 } });
  await dj.waitFor((s) => s.crate[1].plan.cueIn === 0 && s.crate[1].plan.kind === 'crossfade');
  const ids = dj.state.crate.map((v) => v.id);
  ...
  ok('cold start loads the first unplayed crate item onto deck A and plays it',
  ...
  ok('prepping stamps the item played instead of removing it',
    dj.state.crate.length === 3 && unplayed(dj.state).length === 1 && unplayed(dj.state)[0].id === ids[2],
    JSON.stringify({ crate: dj.state.crate.length, unplayed: unplayed(dj.state).map((v) => v.title) }));
  ...
  ok('the outgoing deck is reloaded with the next unplayed crate item, paused at its cue-in',
  ...
  ok('the whole crate has now been played', unplayed(dj.state).length === 0 && dj.state.crate.length === 3,
    JSON.stringify({ crate: dj.state.crate.length, unplayed: unplayed(dj.state).length }));
  ...
  // --- an exhausted crate just stops: the last track plays out and the spare deck stays ejected
  await dj.waitFor((s) => s.decks[1].video === null, 8000);
  ok('with nothing unplayed left the outgoing deck is left ejected', dj.state.decks[1].video === null);
  ...
  for (const item of dj.state.crate) dj.cmd({ action: 'crate.remove', id: item.id });
  await dj.waitFor((s) => s.crate.length === 0);
  dj.cmd({ action: 'crate.addMany', videos: [track('Z1')] });
  await dj.waitFor((s) => s.crate.length === 1);
  ...
  ok('auto-advance is inert while disabled',
    dj.state.mixer.auto.active === false && unplayed(dj.state).length === 1 && dj.state.decks[1].video === null,
    JSON.stringify({ auto: dj.state.mixer.auto.active, unplayed: unplayed(dj.state).length, deckB: dj.state.decks[1].video }));
